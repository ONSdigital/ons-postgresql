...
#--------------------------------------------------------------
#
# NAME:    export_pg_schema.sh
#
# PURPOSE: Exports a schema or group of schemas from the 
#          postgres database.
#
...
# 1.0      16/09/14     S Bennett       Created
#--------------------------------------------------------------
...
function EXPORT_PG_SCHEMA
{
  ...
  #Export the schema
  pg_dump -h $dHOST -p $dPORT -n $dSCHEMA $dDBNAME -f $DUMPFILE
  printf "\n PROGRESS: %s\n" $DUMPFILE
